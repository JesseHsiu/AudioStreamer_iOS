...
- (void)defaults {
    ...
    _labelFont = [UIFont systemFontOfSize:30.0f];
    _snapToLabels = NO;
    _handleType = EFDoubleCircleWithOpenCenter;
    _labelColor = _filledColor;
...
- (void)drawRect:(CGRect)rect
{
    ...
    if((_handleType == EFDoubleCircleWithClosedCenter || _handleType == EFDoubleCircleWithOpenCenter) && fixedAngle > 5) {
        CGContextAddArc(ctx, self.frame.size.width/2  , self.frame.size.height/2, self.radius, 1*M_PI/2, 3*M_PI/2-ToRad(angle+3), 0);
    } else {
        CGContextAddArc(ctx, self.frame.size.width/2  , self.frame.size.height/2, self.radius, 1*M_PI/2, 3*M_PI/2-ToRad(angle), 0);
    }
